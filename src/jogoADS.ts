let nickjogo : string = 'Dragon Egg';
let nomeguerreiro : string | null = null;
let qporcoes : number = 4; 
let controlarAcesso: number = 1;
let continuar: string;
let escolherTurno: string;
...
function perguntaEObriga(pergunta:string,r1:string,r2:string,r3:string):string{
    let resposta : string | null = String(prompt(pergunta)?.trim().toLowerCase());
    while(resposta!=r1 && resposta!=r2 && resposta!=r3){
        alert('Resposta Inválida');
        resposta = String(prompt(pergunta)?.trim().toLowerCase());
    }
    return resposta;
}
...
function ajudarLobo(){
    while(1){
        let ajudarCachorro: string | null = String(prompt('Durante o percurso até a caverna, ouviu-se um gemido baixo e sofrido que ecoou entre as árvores. Ao se aproximar você encontra uma criatura: um lobo preso em uma armadilha. Seu pelo cinza estava manchado de sangue e seus olhos amarelos estavam cheios de dor. Ajudar o lobo? \n- Sim \n- Não')?.trim().toLowerCase());
...
function tentarNovamente(){
    continuar = String(prompt('Tentar novamente? \n- Sim \n- Não')?.trim().toLowerCase());
    if(continuar == 'sim'){
        controlarAcesso = 1;
        qporcoes = 4;
        DragonEgg();
...
function escolherOvo(){
    ...
    let ovoEscolher: string = perguntaEObriga('Escolha um ovo: \n- Esquerda \n- Meio \n- Direita', 'esquerda', 'meio', 'direita');
...
function manha(){
    ...
    let observar: number = 1;
    let desbloquear: number = 1;
    let esperar: string = 'sim';
    while(1) {
        ...
        if(manhaEscolha1 == 'esperar'){
            alert('Esperando...');
            while(observar){
                esperar = String(prompt('Continuar a esperar? \n- Sim \n- Não'))?.trim().toLowerCase();
                if(esperar == 'sim'){
                    desbloquear +=1;
                    alert('Esperando...');
                    if(desbloquear == 3){
            ...
            let escolhaNinho: string = perguntaEObriga('Escolha uma das opções: \n- Caminhar \n- Correr \n- Voltar', 'caminhar', 'correr', 'voltar');
...
function tarde(){
    ...
    while(1) {
        ...
        let tardeEscolha1 = perguntaEObriga('Escolha umas das opções para avançar: \n- Esperar \n- Entrar \n- Desistir', 'esperar', 'entrar', 'desistir')?.toLowerCase();
        if(tardeEscolha1 == 'esperar'){
            while(observar){
                ...
                } else if (esperar?.toLowerCase() == 'não'){
                    break;
                } else {
                    alert('Resposta inválida. Tente novamente!')
                }
...
function noite(){
    ...
    while(1){
        let acenderTocha = String(prompt('Hoje, a lua não apareceu no céu... \nA escuridão reina nessa floresta mágica, então fique alerta! \nAcender uma tocha? \n- Sim \n- Não')?.trim().toLowerCase());
        if(acenderTocha == 'não'){
            ...
            let escolhaCaminhar: string = String(prompt('Ao chegar na caverna, você encontrou o ninho e o dragão. Você quer esperar? \n- Sim \n- Não'))?.trim().toLowerCase();
